...
//  var motion4="translate(1px, 1px)";
  ...
  var mot7 = ffRectangle("txt0", {
   ...
   ffMotion(                                            //텍스트 모션 적용
     {func : mot1, delay : 1, duration : 3, start_time: 0},
     {func : mot8, delay : 5, duration : 2, start_time: 1}
...
console.log("offsetArray size "+offsetArray[0].length);
   for(var i=0;i<offsetArray.length;i++){
     ...
     for(var j=0;j<offsetArray[i].length;j++){
       ...
       if(motionName=="Line"){
         //> setOffset_Line(offset_start, offset_end, left_px, top_px);
          var left_px= targetArr[i][motion_count].func.toLeft;
          var top_px= targetArr[i][motion_count].func.toTop;
          lineMotionArr[i].push({
            offset:startOffset,
            transform: empty_translate
          });
          var line_translate = "translate("+left_px+"px,"+top_px+"px)";
          lineMotionArr[i].push({
            offset:endOffset,
            transform: line_translate
          });
          var finishOffset= endOffset+offset_per_second[i];
          lineMotionArr[i].push({
            offset:finishOffset,
            transform: empty_translate
          });
       ...
       //offset만드는건 따로 함수로 빼기
        ...
        rotationMotionArr[i]=setOffset_Rotation(startOffset, endOffset, angle,rotationMotionArr[i],offset_per_second[i],rotate_axis);
        motion_count++;
       }
        ...
        shakeMotionArr[i]=setOffset_Shake(startOffset, endOffset, shake_count,shake_duration,shakeMotionArr[i],topArr_shake,leftArr_shake,offset_per_second[i]);
        motion_count++;
       }
        ...
        appearMotionArr[i]=setOffset_Appear(startOffset, endOffset,appearMotionArr[i], appear_fromTop,appear_fromLeft,appear_toTop,appear_toLeft,appearOpacity,offset_per_second[i]);
        motion_count++;
       }
        ...
        blinkMotionArr[i]=setOffset_Blink(startOffset, endOffset,blinkMotionArr[i], blink_duration,blink_opacity,num_per_second);
        motion_count++;
       }
        ...
        circleMotionArr[i]=setOffset_Circle(startOffset, endOffset,circleMotionArr[i],circle_leftArr,circle_topArr,offset_per_second[i]);
        motion_count++;
       }
        ...
        rectangleMotionArr[i]=setOffset_Rectangle(startOffset, endOffset,rectangleMotionArr[i],rectangle_leftArr,rectangle_topArr,offset_per_second[i]);
        motion_count++;
       }
       else if(motionName=="Scale"){
        var scaleX= targetArr[i][motion_count].func.scaleX;
        var scaleY= targetArr[i][motion_count].func.scaleY;
        scaleMotionArr[i]=setOffset_Scale(startOffset, endOffset,scaleMotionArr[i],scaleX,scaleY,offset_per_second[i]);
        motion_count++;
       }
     }
   }
  ...
  scale_target.style.position='fixed';
  scale_target.style.left='200px';
   scale_target.style.top='200px';
...
/////////////////////////모션별 offset 설정 함수/////////////////////////////////
var setOffset_Scale = function(startOffset, endOffset,motionArr,scaleX,scaleY,offset_gap){
  ...
  motionArr.push({offset:endGap, transform:scale_transform});
  motionArr.push({offset:endOffset, transform:empty_scale});
  return motionArr;
};

var setOffset_Rectangle = function(startOffset, endOffset,motionArr,leftArr,topArr,offset_gap){
  ...
  //offset이 증가하는 크기
  var offset_interval = offsetGap/arrSize;
  var rectangle_transform = 'translate('+leftArr[0]+'px,'+topArr[0]+'px)';
  motionArr.push({offset:startOffset, transform: empty_rectangle});
  motionArr.push({offset:startGap , transform:rectangle_transform});

  for(var i=1;i<arrSize;i++){
    rectangle_transform = 'translate('+leftArr[i]+'px,'+topArr[i]+'px)';
    console.log(leftArr[i]+ " "+topArr[i]);
    var rectangle_offset = startOffset+i*offset_interval;
    motionArr.push({offset:rectangle_offset , transform:rectangle_transform});
  }
  rectangle_transform = 'translate('+leftArr[arrSize-1]+'px,'+topArr[arrSize-1]+'px)';
  //motionArr.push({offset:endGap , transform:rectangle_transform});
  //motionArr.push({offset:endOffset, transform: empty_rectangle});
  return motionArr;
};

var setOffset_Circle = function(startOffset, endOffset,motionArr,leftArr,topArr,offset_gap){
  ...
  //offset이 증가하는 크기
  var offset_interval = offsetGap/arrSize;
  var circle_transform = 'translate('+leftArr[0]+'px,'+topArr[0]+'px)';
  motionArr.push({offset:startOffset, transform: empty_circle});
  motionArr.push({offset:startGap , transform:circle_transform});

  for(var i=1;i<arrSize-1;i++){
    circle_transform = 'translate('+leftArr[i]+'px,'+topArr[i]+'px)';
    //console.log(leftArr[i]+ " "+topArr[i]);
    var circle_offset = startOffset+i*offset_interval;
  ...
  motionArr.push({offset:endGap , transform:circle_transform});
  motionArr.push({offset:endOffset, transform: empty_circle});
  return motionArr;
};

var setOffset_Blink = function(startOffset, endOffset,motionArr,duration,opacity,num_per_second){

  var blink_num=duration * num_per_second*2;
  var offsetGap = endOffset - startOffset;
...
var setOffset_Appear = function(startOffset, endOffset,motionArr,fromTop,fromLeft,toTop,toLeft,opacityArr,offset_gap){
  ...
  motionArr.push({offset:startGap , transform:appear_transform,opacity:opacityArr[0]});
  appear_transform = 'translate('+toLeft+'px,'+toTop+'px)';
  motionArr.push({offset:endGap , transform:appear_transform,opacity:opacityArr[1]});
  motionArr.push({offset:endOffset, transform: empty_appear});
  return motionArr;
};
var setOffset_Shake = function(startOffset, endOffset,shake_count,duration,motionArr,topArr,leftArr,offset_gap){
  ...
  var totalShakeNum = shake_count*2*duration;
...
var setOffset_Line = function(startOffset, endOffset, leftPx, topPx){
  var lineOffset = new Array();

};
var setOffset_Rotation = function(startOffset, endOffset,angle, motionArr,offset_gap,axis){
  ...
  motionArr.push({offset:gapOffset , transform:rotate_transform,transformOrigin: axis});
  motionArr.push({offset:endOffset, transform: empty_rotate});
  return motionArr;
}
/////////////////////////////////////////////모션 속성 관련/////////////////////////////////////////////
//다 start_time 추가해주기
   ...
   for(var p1=0; p1<len_target; p1++){
      ...
      for(var p2=0; p2<offset_length; p2++){
        ...
        if(p2==offset_length-1){
          offsetArray[p1][p2] = {motion_name:motion_name, target:target_name,start_offset:start_offset, end_offset: 1};
          //console.log(offsetArray[p1][p2]);
        }
        else{
          offsetArray[p1][p2] = {motion_name:motion_name, target:target_name,start_offset:start_offset, end_offset:end_offset};
        }
      }
   }
...
/////////////////////////////////모션의 전체 실행시간 계산//////////////////////////////////////////
    ...
    for(var p1=0; p1<len_target; p1++){
      ...
      for(var p2=0; p2<count_motion; p2++){
        ...
        if(p2==0){
          ...
          relativeStartTime[p1][start_count] = {motion_name:motion_name, target:target_name,time:start_time, duration:duration_arr[p1][p2]};
          start_count++;
        }
        else{
          ...
          if(total_time<= start_arr[p1][p2]){
           ...
           var empty_startTime=total_time[p1];
            relativeStartTime[p1][start_count] = {motion_name:empty_name,target:target_name, time:total_time[p1], duration:empty_time};
            start_count++;
 ...
 ///////////////////////////////////////텍스트 바인딩////////////////////////////////////
 function setTextBinding(upper, lower){
